...
public class Diameter_Of_A_Binary_Tree {
  ...
  public static int diameter1(Node node) {
    ...
    int lhs = diameter1(node.left);
    //maximum distance between two nodes of rhs
    int rhs = diameter1(node.right);
    // maximum distance between  left's deepst & right's deepest
    int f = height(node.left) + height(node.right) + 2;
    
    int dia = Math.max(f, Math.max(lhs, rhs));
    return dia;
  }
